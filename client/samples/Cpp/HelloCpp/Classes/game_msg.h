...
namespace msg_type {
    ...
    const msg_type_t despawn = 1003;
    const msg_type_t world_info = 1004;
    const msg_type_t chat = 1005;
    const msg_type_t character_resource = 1006;
...
struct move_msg {
    ...
    move_msg(int _id, double _x, double _y, double _dir, double _speed, double _time, bool _instance_move)
        : id(_id), x(_x), y(_y), dir(_dir), speed(_speed), time(_time), instance_move(_instance_move) {}
    
    #ifdef _DEBUG
    move_msg()
        : id(0), x(0), y(0), dir(0), speed(0), time(0), instance_move(false) {}
    #else
    move_msg() {}
    #endif
...
struct chat_msg {
    ...
    chat_msg(int _id, std::string _name, std::string _message)
        : id(_id), name(_name), message(_message) {}
...
struct character_resource_msg {
    ...
    character_resource_msg(int _id, int _resource_id, int _color, int _radius)
        : id(_id), resource_id(_resource_id), color(_color), radius(_radius) {}
    
    #ifdef _DEBUG
    character_resource_msg()
        : id(0), resource_id(0), color(0), radius(0) {}
    #else
    character_resource_msg() {}
    #endif
...
struct world_info_msg {
    ...
    world_info_msg(int _id, int _world_id, double _server_now)
        : id(_id), world_id(_world_id), server_now(_server_now) {}
    
    #ifdef _DEBUG
    world_info_msg()
        : id(0), world_id(0), server_now(0) {}
    #else
    world_info_msg() {}
    #endif
...
inline void msg::spawn_msg::handle(msg_session_ref session, const type& msg)
{
    ...
    msg::move_msg::handle(session, msg.move);
