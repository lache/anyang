...
bool CCTexture2DMutable::initWithData(const void* data, Texture2D::PixelFormat pixelFormat, unsigned int width, unsigned int height, const Size& size)
{
	switch (pixelFormat) {
	...
	int mem = width*height*bytesPerPixel_;
	if (!Texture2D::initWithData(data, mem, pixelFormat, width, height, size)) {
		return false;
	}
...
Color4B CCTexture2DMutable::pixelAt(const Point& pt)
{
	Color4B c = { 0, 0, 0, 0 };
	if (!data_) return c;
...
bool CCTexture2DMutable::setPixelAt(const Point& pt, Color4B c)
{
...
void CCTexture2DMutable::fill(Color4B p)
{
...
Texture2D* CCTexture2DMutable::copyMutable(bool isMutable)
{
	Texture2D* co;
	if (isMutable)
	{
		...
		if (!co->initWithData(data_, mem, _pixelFormat, _pixelsWide, _pixelsHigh, _contentSize)) {
			delete co;
			co = NULL;
		}
	}
...
void CCTexture2DMutable::copy(CCTexture2DMutable* textureToCopy, const Point& offset)
{
	for (int r = 0; r < _contentSize.height; ++r){
		for (int c = 0; c < _contentSize.width; ++c){
